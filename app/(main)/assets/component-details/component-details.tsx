...
const ComponentData = () => {
  ...
  const tabMenuTemplate = () => {
    if (tabMenu.top == 'data') {
      if (tabMenu.bottom == 'general') return <DataGeneral />;
      else if (tabMenu.bottom == 'operating') return <DataOperating />;
      else if (tabMenu.bottom == 'inspection') return <DataInspection />;
    } else if (tabMenu.top == 'other_damages') {
      if (tabMenu.bottom == 'damage_factor') return;
      else if (tabMenu.bottom == 'damage_category') return;
    } else if (tabMenu.top == 'probability') {
      if (tabMenu.bottom == 'pof_rbi_date') return <POFRBIDate />;
      else if (tabMenu.bottom == 'pof_plan_date') return <POFPlanDate />;
    }
  };
  ...
  return (
    <Card subTitle="Data" className="shadow-1 border-round-xl">
      ...
      <div className="p-3">{tabMenuTemplate()}</div>
    </Card>
  );
};
