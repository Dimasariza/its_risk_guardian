...
export const calculateThinning = (generalData: IGeneralData, thinning: IRBIThinning) => {
    ...
    const age: number | null= rbiThinning_rbiDate?.getFullYear() - lastInspDateObj?.getFullYear() || null;
...
export const NORMSDIST = (z: number) => {
    return 1 / ((2 * Math.PI) * - Math.exp(z ** 2 / 2))
}
